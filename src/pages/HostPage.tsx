  ...
  if (game.status === "setup") {
    return (
      <div className="h-dvh flex flex-col overflow-hidden">
        ...
        <button
          className="bg-[#6b93a6] text-white rounded-[10px] p-2 shadow-sm transition-transform duration-300 ease-out hover:scale-95 cursor-pointer font-mono m-4 shrink-0"
          onClick={startGame}
        >
          Start Game
        </button>
  ...
  return (
    <div>
      ...
      {game.status === "in_progress" && revealed && (
        <div className="h-dvh flex flex-col overflow-hidden">
          <h1 className="text-center pt-4 m-0 shrink-0">Jeoparty</h1>
          <p className="text-center my-8 text-[var(--label-text)] text-sm font-mono uppercase shrink-0">
            {revealed.category.name || "Untitled"} {revealed.question.points}
          </p>
          <div
            className={`flex-1 min-h-0 flex flex-col items-center gap-2 px-6 pb-6 ${
          ...
          >
            ...
            {revealed.question.state === "judging" && (
              <div className="w-full flex-1 min-h-0 flex flex-col gap-2 mt-6">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-2">
                  {submissions.length === 0 ? (
                    ...
                    submissions.map((submission) => (
                      <label
                        key={submission.id}
                        className="flex items-center gap-3 bg-[#eeeeee] rounded-[10px] p-3 leading-none shrink-0"
                      >
                        <input
                          type="checkbox"
                ...
                <button
                  className="bg-[#6b93a6] font-mono text-white rounded-[10px] p-2 shadow-sm transition-transform duration-300 ease-out hover:scale-95 cursor-pointer w-full shrink-0"
            ...
            {revealed.question.state === "answered" && (
              <div className="w-full flex-1 min-h-0 flex flex-col gap-2 mt-6">
                ...
                <h2 className="text-3xl text-[var(--correct)] shrink-0">
                  {revealed.question.correct_answer}
                </h2>
                <label className="text-[var(--label-text)] text-sm font-mono uppercase mb-2 mt-4 shrink-0">
                  Who Got It Right?
                </label>
                ...
                <button
                  className="bg-[#6b93a6] text-white rounded-[10px] p-2 shadow-sm transition-transform duration-300 ease-out hover:scale-95 cursor-pointer font-mono shrink-0"
